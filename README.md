...
起動後、表示される `http://<LANのIPアドレス>:3000` を同じネットワーク内の端末で開きます。`PORT`、`MAX_UPLOAD_MB`、`OPEN_BROWSER=0` の環境変数で動作を変更できます。
...
このプロジェクトのソースは、同梱されていた `lan-file-share.exe` のSEAリソースから抽出しました。再抽出が必要な場合は以下を実行できます。

```powershell
.\tools\extract-sea.ps1
```

元のEXE、生成物、共有データ、`node_modules` はGitの追跡対象外です。
